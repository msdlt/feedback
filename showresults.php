...
<?php 
	...
	for($i=1;$i<=$noOfCriteria;$i++)
		...
		$qQuestionItems = "SELECT QuestionItems.itemID, QuestionItems.position, Items.text as itemText, Questions.text as questionText
						...
						((isset($showHidden) && $showHidden=='on')?" " : " AND QuestionItems.visible = 1 ").
...
if(IsAuthor($heraldID))
	...
	echo "<a href=\"results.php\">Results</a> &gt; <a href=\"viewresults.php\">View Results</a> &gt; <strong>$surveyTitle - Results</strong>";
	...
	while($rowSections = mysqli_fetch_array($qResSections))
		...
		while($rowQuestions = mysqli_fetch_array($qResQuestions))
			...
			if($bAnalyseThisQuestion == true)
				{
				if(($blockVisible==1 && $sectionVisible==1 && $questionVisible==1) || $showHidden=='on') echo "<table class=\"matrix\" border=\"0\" cellpadding=\"2\" cellspacing=\"0\">"; 
				$NoOfColumns = count($aQuestionToAnalyseByItems[$criterionNumber]) + 1;
				if(($blockVisible==1 && $sectionVisible==1 && $questionVisible==1) || $showHidden=='on') echo "<tr class=\"matrixHeader\"> <th colspan=\"" . $NoOfColumns . "\">" . $questionNo.". ".$rowQuestions['text'] . " - analysed by: " . $aQuestionToAnalyseByItems[$criterionNumber][$i][1] . "</th></tr>";
				...
				$aRowHTMLTopRow = "<tr class=\"matrixHeader\">
										<th>Item</th>";
				$aRowDates = "<tr class=\"$rowClass\"><td></td>";
				...
				for ($i=0;$i<count($aQuestionToAnalyseByItems[$criterionNumber]);$i++)
					...
					$aRowHTMLTopRow = $aRowHTMLTopRow . "<th valign=\"top\">" . $aQuestionToAnalyseByItems[$criterionNumber][$i][2] . "</th>";
					...
					while($rowItems = mysqli_fetch_array($qResItems))
						...
						if($i==0)
							...
							$aRowHTMLByItem[$itemNo] = "<tr class=\"$rowClass\">
																<td>".$rowItems['text']."</td>
																<td>$PercentageOfAnswers ($NoOfItems)</td>";
							}
						else
							{
							$aRowHTMLByItem[$itemNo] = $aRowHTMLByItem[$itemNo] . "<td>$PercentageOfAnswers ($NoOfItems)</td>";
							}
						$itemNo++;
						$bRowOdd = !$bRowOdd;
						}
					$aRowDates = $aRowDates . "<td>";
					while($rowDates = mysqli_fetch_array($qResDates))
						{
						$aRowDates = $aRowDates . ODBCDateToTextDateShort($rowDates['date']). "<br/>";
						}
					$aRowDates = $aRowDates . "</td>";
					}
				...
				if(($blockVisible==1 && $sectionVisible==1 && $questionVisible==1) || $showHidden=='on') echo "<table class=\"matrix\" border=\"0\" cellpadding=\"2\" cellspacing=\"0\">"; 
				$aRowHTMLTopRow = "<tr class=\"matrixHeader\">
										<th>$questionNo. ".$rowQuestions['text']."</th>";
				$aRowHTMLByItem = "<tr><td></td>";
				...
				while($rowItems = mysqli_fetch_array($qResItems))
					...
					$aRowHTMLTopRow = $aRowHTMLTopRow . "<td>".$rowItems['text']."</td>"; 
					$aRowHTMLByItem = $aRowHTMLByItem . "<td>$PercentageOfAnswers ($NoOfItems)</td>";
					}
				...
				if(($blockVisible==1 && $sectionVisible==1 && $questionVisible==1) || $showHidden=='on') 
					...
					echo "<tr>
							<td colspan=\"$colSpan\">
								<div class=\"comments\" id=\"divComments_".$questionNo."\" name=\"divComments_".$questionNo."\">
...
<?php
	...
	if($surveyAllowSave == "true")
		{
				echo "<p>If you do not wish to complete the whole form in one go, click the <strong>Save</strong> button to save your answers 
						so far. However, in order for your feedback to be counted, you will need to complte all the questions and submit it finally by clicking the
						<strong>Submit</strong> button before the closing date.</p>";
		}
	else
		{
				echo "<p>Please complete all questions before clicking the <strong>Submit</strong> button.</p>";
		}
			echo "<p>You may revisit your answers at any time. However, you will not be able to make changes after the closing date.</p>";
			echo "</div>
			</div>";*/
?>
</body>
